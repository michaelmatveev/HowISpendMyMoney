...
//		}
	...
	Word[] getWordsAt(float x, float y, float radius) {
		ArrayList<Word> result = new ArrayList<Word>();
		Path targetPath = new Path();
		targetPath.addCircle(x, y, radius, Path.Direction.CW);
		RectF bounds = new RectF();
		RectF targetBounds = new RectF();
		targetPath.computeBounds(targetBounds, false);	
...
//path.offset(currentLocation.x, currentLocation.y);
		...
		for (int i = 0; i < eWords.length; i++) {
			if (eWords[i].wasPlaced()) {
				Path shape = eWords[i].getPath();
				
				shape.computeBounds(bounds, false);
				AVector d = eWords[i].word.getRenderedPlace();
				bounds.offset(d.x, d.y);
				//if(shape.op(targetPath, Op.INTERSECT)) {
				if(bounds.intersect(targetBounds)) {
					result.add(eWords[i].word);
				}
			}
		}
